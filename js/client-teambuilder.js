(function (exports, $) {
	...
	exports.TeambuilderRoom = exports.Room.extend({
		...
		updateFolderList: function () {
			...
			if (Storage.teams) for (var i = -2; i < Storage.teams.length; i++) {
				...
				if (i === -2) {
					...
					if (!format) format = 'gen7lucklessou';
				}
				if (!format) continue;
		...
		updateTeamList: function (resetScroll) {
			...
			if (filterFormat && filterFormat !== 'gen7lucklessou') {
			...
			if (!teams.length) {
				...
				for (var i = 0; i < teams.length + 1; i++) {
					...
					if (filterFormat && filterFormat !== (team.format || 'gen7lucklessou')) continue;
		...
		pokepasteExport: function () {
			...
			if (this.curTeam.format !== 'gen7lucklessou') document.getElementById("pasteNotes").value = "Format: " + this.curTeam.format;
		...
		updateChart: function (pokemonChanged, wasIncomplete) {
			...
			if (pokemonChanged || this.search.qName !== this.curChartName) {
				...
				this.search.setType(type, this.curTeam.format || 'gen7lucklessou', this.curSet, cur);
				this.qInitial = q;
